...
# -*- coding: utf-8 -*-
"""
MacOS Memory Priority Manager - Versione avanzata
...
Funzionalità:
- Evita di bloccare Terminal.app e processi bash
- Evita di bloccare il processo padre dell'applicazione che esegue lo script
- Supporta input interattivo per aggiungere/rimuovere processi dalla lista di esclusione
- Integra funzionalità macOS specifiche per la gestione della memoria quando disponibili
"""
...
class ProcessInfo:
    def __init__(self, pid, name, memory_mb, cpu_percent):
        self.pid = pid
        self.name = name
...
def is_essential(process, parent_chain_pids, user_excluded_pids, current_app_name=None):
    ...
    Args:
        process: Oggetto ProcessInfo
        parent_chain_pids: Lista di PID della catena dei processi genitori 
        user_excluded_pids: Set di PID esclusi dall'utente
        current_app_name: Nome dell'applicazione che esegue lo script
    """
...
def force_process_swap(pid):
    ...
    try:
        ...
        if os.geteuid() == 0:  # Verifica se siamo root
            ...
            # Utilizzo di vm_pageout per forzare lo swap di pagine specifiche
            # Questo è un comando avanzato e potrebbe richiedere autorizzazioni speciali
...
def memory_manager(priority_pid, min_memory_threshold, max_pause_count, check_interval, 
                  force_swap, user_excluded_pids, stop_event):
    """
    ...
    try:
        while not stop_event.is_set():
            ...
            if current_time - last_check_time >= check_interval:
                ...
                if len(paused_processes) < max_pause_count:
                    ...
                    for process in high_memory_processes:
                        if len(paused_processes) >= max_pause_count or stop_event.is_set():
                            break
                        
                        # Verifica se il processo è già in pausa
                        if any(p.pid == process.pid for p in paused_processes):
                            continue
            ...
            # Breve pausa per evitare di consumare troppe risorse
            time.sleep(0.5)
                
    except Exception as e:
        print(f"\nErrore nel gestore di memoria: {e}")
    finally:
        # Ripristina tutti i processi messi in pausa
